...
export const newIncomerInvestorController = async (req, res) => {
    try {
        const { investorEmail, investorName, category, country, state, city, investmentRange } = req.body;
...
// Prepare query conditions
        ...
        console.log(`Found ${perfectMatches.length} perfect matches for investor: ${investorEmail}`);
        ...
        for (const brand of perfectMatches) {
            ...
            if (!emailedBrands.has(brandEmail)) {
                try {
                    ...
                    console.error(`Failed to email perfect match: ${brandEmail}, error`, error);
                }
            }
        }
        ...
        for (const brand of partialMatches) {
            const brandEmail = brand.personalDetails.email;
            const brandCompanyName = brand.personalDetails.companyName;
            const brandCategory = brand.personalDetails.brandCategories[0]?.child || "Not specified";
            ...
            if (!emailedBrands.has(brandEmail)) {
                try {
                    ...
                    console.error(`Failed to email partial match: ${brandEmail}, error`, error);
                }
            }
        }
