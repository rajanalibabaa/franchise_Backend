...
export const toggleBrandPausePlay = async (req, res) => {
  try {
    const {id} = req.params
    ...
    console.log(exists.brandDetails.isBrandPause)

    const data = await BrandDetails.findByIdAndUpdate(
            exists._id,
            { $set: { "brandDetails.isBrandPause": !exists?.brandDetails.isBrandPause } },
            { new: true }
        );

    let message
    if (data.brandDetails.isBrandPause === true) {
        message ="Brand pause successfully"
    } else {
        message ="Brand play successfully"
    }
...
export const getAllPauseBrand = async (req, res) => {
  try {
    ...
    const aggregationPipeline = [
      {
        $match: {
          "brandDetails.isBrandPause": { $ne: false } 
        }
      },
      {
        $lookup: {
    ...
    const [brandsData, totalCountResult] = await Promise.all([
      ...
      BrandDetails.aggregate([
        {
          $match: { "brandDetails.isBrandPause": { $ne: false } } 
        },
        {
          $count: "totalCount"
        }
