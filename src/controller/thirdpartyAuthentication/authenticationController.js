...
const googleAuthProfile = async (req, res) => {
  if (req.isAuthenticated()) {
    ...
    const exists = await ThirdPartyAuth.findOne({
      $and: [{ email: email }, { source: "google" }],
    });
    if (exists) {
      return res.json(new ApiResponse(409, null, " user already exists"));
    }
    ...
    if (!data) {
      return res.json(new ApiResponse(400, null, "user not created"));
    }
...
const facebookAuthProfile = async (req, res) => {
  if (req.isAuthenticated()) {
    ...
    const exists = await ThirdPartyAuth.findOne({
      $and: [{ email: email }, { source: "facebook" }],
    });
    if (exists) {
      return res.json(new ApiResponse(409, null, " user already exists"));
    }
...
export { googleAuthProfile, facebookAuthProfile };
