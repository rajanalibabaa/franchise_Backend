...
class ManualLeadController {
  ...
  async getLeadByUUID(req, res) {
    try {
      ...
      const lead = await ManualLead.findOne({ uuid, isActive: true }).populate(
        "assignedTo",
        "name email",
      );
    ...
    try {
      ...
      if (updateData.email || updateData.mobileNumber) {
        ...
        if (updateData.mobileNumber)
          duplicateQuery.mobileNumber = updateData.mobileNumber;
        ...
        if (duplicate) {
          return res.status(409).json({
            success: false,
            message:
              "Another lead with this email or mobile number already exists",
          });
        }
      }
