...
export const recievingOtherIndustriesData = async (req, res) => {
  ...
  if (!name || !email || !mobileNumber || !registerAs || !category) {
    return res.json(new ApiResponse(404, {}, "All the details are required"));
  }
  ...
  if (exists) {
    return res.json(new ApiResponse(404, null, "Already Exists"));
  }
  ...
  return res.json(new ApiResponse(200, {}, "Successfully submitted"));
};
