  ...
  try {
    ...
    const prefData =
      preferences?.map((pref, index) => {
        if (
          !pref.locationType ||
          !["domestic", "international"].includes(pref.locationType)
        ) {
          throw new Error(
            `Invalid or missing 'locationType' in preference ${index + 1}`,
          );
        }
        ...
        if (
          ...
          pref.category.length === 0
        ) {
          throw new Error(
            `At least one category is required in preference ${index + 1}`,
          );
        }
        ...
        const propertyPreferred = (pref.propertyPreferred || []).map(
          (prop) => ({
    ...
    if (existingInvestor) {
      return res
        .status(409)
        .json(new ApiResponse(409, null, "Investor already exists"));
    }
...
export const getInvestorByUUID = async (req, res) => {
  try {
    ...
    const investor = await InvsRegister.findOne({
      uuid: req.investorUser?.uuid || uuid,
    }).select("-__v -_id -createdAt -updatedAt -oldData");

    if (!investor) {
      return res
        .status(404)
        .json(new ApiResponse(404, null, "Investor not found"));
    }
...
export const updateInvestor = async (req, res) => {
  try {
    ...
    const oldData = await InvsRegister.findOne({
    ...
    if (rawPreferences !== undefined) {
      ...
      processedPreferences = parsedPreferences.map((pref, index) => {
        ...
        if (!pref.investmentRange || !pref.investmentAmount) {
          throw new Error(
            `Preference ${index + 1} is missing required investment fields`,
          );
        }

        if (
          !Array.isArray(pref.propertyPreferred) ||
          pref.propertyPreferred.length === 0
        ) {
          throw new Error(
            `At least one property preference is required in preference ${index + 1}`,
          );
        }
        ...
        pref.propertyPreferred.forEach((prop, propIndex) => {
          if (!prop.propertyType) {
            throw new Error(
              `Property type is required in property preference ${propIndex + 1} of preference ${index + 1}`,
            );
          }

          if (prop.propertyType === "Own Property" && !prop.propertySize) {
            throw new Error(
              `Property size is required for Own Property in property preference ${propIndex + 1} of preference ${index + 1}`,
            );
          }
        });
        ...
        if (pref.locationType === "domestic") {
          if (
            ...
            throw new Error(
              `Domestic location fields (state, district, city) are required in preference ${index + 1}`,
            );
          }
        } else if (pref.locationType === "international") {
          if (!pref.preferredCountry || !pref.preferredCity) {
            throw new Error(
              `International location fields (country, city) are required in preference ${index + 1}`,
            );
          }
        }
        ...
        if (
          !pref.locationType ||
          !["domestic", "international"].includes(
            pref.locationType.toLowerCase(),
          )
        ) {
          throw new Error(
            `Invalid or missing locationType in preference ${index + 1}`,
        ...
        const processedPropertyPreferred = pref.propertyPreferred.map(
          (prop) => ({
            propertyType: String(prop.propertyType),
            propertySize:
              prop.propertyType === "Own Property"
                ? String(prop.propertySize || "")
                : "",
        ...
        return {
          ...
          _id: mongoose.isValidObjectId(pref._id)
            ? pref._id
      ...
      preferencesChanged =
        JSON.stringify(processedPreferences) !==
    ...
    if (req?.file?.path) {
      ...
      profileImage = await uploadFileToR2(
        req.file.path,
      ...
      if (!profileImage) {
        return res
          .status(500)
          .json(new ApiResponse(500, null, "Failed to upload profile image"));
      }
      imageChanged = true;
    }
        ...
        return res
          ...
          .json(
            new ApiResponse(
              400,
              null,
              "Please specify your occupation when selecting 'Other'",
            ),
          );
      }
    ...
    if (preferencesChanged) {
      ...
      const sanitizedOldPreferences = (oldData.preferences || []).map(
        (pref) => ({
          ...
          propertyPreferred: Array.isArray(pref.propertyPreferred)
            ? pref.propertyPreferred.map((prop) => ({
                propertyType: prop.propertyType || "",
                propertySize: prop.propertySize || "",
                propertyCountry: prop.propertyCountry || "",
                propertyState: prop.propertyState || "",
    ...
    if (
      err.message.includes("Preference") ||
...
export const deleteInvestorProfileImage = async (req, res) => {
  try {
    ...
    if (!investor.profileImage) {
      return res
        .status(400)
        .json(new ApiResponse(400, null, "No profile image exists to delete"));
    }
...
export const deleteInvestor = async (req, res) => {
  ...
  try {
    ...
    if (!deletedInvestor) {
      return res
        .status(404)
        .json({ error: "Investor not found", details: "Investor not found" });
    }

    res
      .status(200)
      .json(new ApiResponse(200, {}, "Investor deleted successfully"));
  } catch (error) {
    res
      .status(500)
      .json({ error: "Failed to delete Investor ", details: error.message });
  }
};