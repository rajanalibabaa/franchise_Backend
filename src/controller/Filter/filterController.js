...
export const getAllBrandsAndFilter = async (req, res) => {
  try {
    ...
    // Merge FICO-related filters into a single $elemMatch so multiple
    // criteria (investmentRange, areaRequired, modelType, franchiseType)
    // are combined instead of overwriting each other.
    const ficoElemMatch = {};
    if (investmentRange) ficoElemMatch.investmentRange = investmentRange;
    if (areaRequired) ficoElemMatch.areaRequired = areaRequired;
    if (modelType) ficoElemMatch.franchiseModel = modelType;
    if (franchiseType) ficoElemMatch.franchiseType = franchiseType;

    if (Object.keys(ficoElemMatch).length) {
      match["franchiseDetails.franchiseDetails.fico"] = {
        $elemMatch: ficoElemMatch,
      };
    }
