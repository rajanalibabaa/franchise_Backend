...
const getAllnewRegisterBrands = async (req, res) => {
  try {
    ...
    return res.json(new ApiResponse(500, null, "Internal server error", error));
  }
};
...
const getbrandsbyCityName = async (req, res) => {
  ...
  try {
    if (!city) {
      const brandsbyCityName = await BrandListing.find({}).select(
        " -_id -__v -createdAt -updatedAt -uuid -personalDetails.email -personalDetails.email -personalDetails.mobileNumber -personalDetails.whatsappNumber -personalDetails.pancardNumber -personalDetails.gstNumber -personalDetails.facebook -personalDetails.instagram  -personalDetails.linkedin -personalDetails.fullName -brandDetails.pancard -brandDetails.gstCertificate ",
      );
      return res.json(
        new ApiResponse(
          200,
          brandsbyCityName,
          " All brands fetch successfully ",
        ),
      );
    }
    ...
    if (!brandsbyCityName) {
      return res.json(
        new ApiResponse(
          400,
          null,
          " No brand registered yet in this city :" + city,
        ),
      );
    }
    ...
    return res.json(
      new ApiResponse(
        200,
        brandsbyCityName,
        " All brands by city name fetch successfully ",
      ),
    );
  } catch (error) {
    return res.json(new ApiResponse(500, null, "Internal server error", error));
...
const getbrandsbyInvestmentRange = async (req, res) => {
  ...
  const projection =
    "-_id -__v -createdAt -updatedAt -uuid " +
  ...
  try {
    if (
      !investmentRangeInput ||
      (Array.isArray(investmentRangeInput) && investmentRangeInput.length === 0)
    ) {
    ...
    const rangeFilter = Array.isArray(investmentRangeInput)
      ? investmentRangeInput
      : [investmentRangeInput];

    const filteredBrands = await BrandListing.find({
      "franchiseDetails.modelsOfFranchise.investmentRange": {
        $in: rangeFilter,
      },
    }).select(projection);
    ...
    if (!filteredBrands || filteredBrands.length === 0) {
      return res.json(
        new ApiResponse(
          404,
          null,
          `No brands found in investment range: ${rangeFilter.join(", ")}`,
        ),
      );
    }
    ...
    return res.json(
      new ApiResponse(
        200,
        filteredBrands,
        "Brands filtered by investment range fetched successfully",
      ),
    );
  } catch (error) {
