...
export const verifyJWT = async (req, res, next) => {
  try {
    ...
    if (!token) {
      return res.json(
        new ApiResponse(401, null, "token not found pleace login"),
      );
    }
    ...
    if (!decodedToken) {
      return res.json(
        new ApiResponse(401, null, "Unauthorized request token not match"),
      );
    }
    ...
    if (decodedToken.adminUUID) {
      admin = await RegisterSuperAdmin.findOne({
        uuid: decodedToken.adminUUID,
      });
    } else if (decodedToken.brandUserUUID) {
      brandUser = await BrandDetails.findOne({
        uuid: decodedToken.brandUserUUID,
      });
    } else if (decodedToken.investorUUID) {
      investorUser = await InvsRegister.findOne({
        uuid: decodedToken.investorUUID,
      });
    } else if (decodedToken.thirdPartyUUID) {
      thirdPartyUser = await ThirdPartyAuth.findOne({
        uuid: decodedToken.thirdPartyUUID,
      });
    } else {
    ...
    if (
      token !== brandUser?.userNewVerifyToken &&
      token !== investorUser?.userNewVerifyToken &&
      token !== thirdPartyUser?.userNewVerifyToken &&
      token !== admin?.userNewVerifyToken
    ) {
      return res.json(new ApiResponse(409, null, "token Expired please login"));
    }
