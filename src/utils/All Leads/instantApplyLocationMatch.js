...
export const instantApplyLocationMatch = async (
  ...
  try {
    ...
    if (!brandBatchDoc.isFreeLeadsBrandPaused) {
      ...
      console.log(" OverAllBrandExists :",OverAllBrandExists.length);
      ...
      if (brandsToSend.length > 0) {
        // console.log("=======brandsToSend.length======")
          ...
          for (const { brand, countDoc } of brandsToSend) {
            ...
            ignoreEmail.push(brand?.brandDetails?.email)
          }
    ...
    if (!brandBatchDoc.isPaidLeadsBrandPaused) {
      ...
      const OverAllBrandExists = await BrandDetails.aggregate(aggregationPipeline);
      // console.log("isPaidLeadsBrandPaused :",OverAllBrandExists)
      if (OverAllBrandExists.length > 0) {
        ...
        const brandsSent = [];
        for (const { brand, countDoc } of brandsToSend) {
          console.log("=====xxxx==== :",brand.brandDetails?.email,
            email,
            mobileNumber,);
